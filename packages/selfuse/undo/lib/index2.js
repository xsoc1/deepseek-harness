/**
 * @dsh-selfuse/undo: undo/rollback system for DeepSeek Harness.
 *
...
let toolsRequire = null;
{
  ...
  if (!toolsRequire) {
    // Local fallback for this machine's source checkout (F:\tools\deepseek-harness).
    try {
      toolsRequire = createRequire('F:/tools/deepseek-harness/package.json');
      toolsRequire.resolve('@deepseek-ai/dsh-tools');
    } catch { toolsRequire = null; }
  }
  if (!toolsRequire) {
    throw new Error('@dsh-selfuse/undo: cannot resolve "@deepseek-ai/dsh-tools". Install the plugin via `dsh plugin add` (peer deps resolve automatically), or set DSH_ROOT to your DSH install root for local junction mounts.');
  }
  try {
...
export const name = '@dsh-selfuse/undo';
...
/**
 * 快照范围的单一事实来源（模块 7：与 tools/@dsh-selfuse/undo-lib.ps1 共用
...
async function safeModeSet(cfg, on) {
  ...
  if (on) {
    ...
    const minimal = `# @dsh-selfuse/undo SAFE MODE (entered ${new Date().toISOString()})\n# All user plugins except @dsh-selfuse/undo are temporarily disabled.\n- insert:\n    - id: @dsh-selfuse/undo\n      name: @dsh-selfuse/undo\n`;
    ...
    return { ok: true, active: true, snapshotId: snap.id, message: `Safe mode ON (pre-snapshot ${snap.id}). Restart DSH to boot with only @dsh-selfuse/undo.` };
  }
  // off
  ...
  if (patchFile) {
    try {
      ...
      for (const m of text.matchAll(/name:\s*['"]?([^'"\s]+)['"]?/g)) {
        ...
        if (n === '@dsh-selfuse/undo') continue; // 自身永远在
        names.add(n);
      }
    } catch { /* patch 缺失则跳过 */ }
  }
  ...
  if (pkgFile) {
    try {
      ...
      for (const n of (pkg.dsh?.profile?.bundles ?? [])) {
        if (typeof n === 'string' && n !== '@dsh-selfuse/undo') names.add(n);
      }
...
async function ensureMount(cfg) {
  ...
  try {
    ...
    bundleMode = Array.isArray(pkg?.dsh?.profile?.bundles) && pkg.dsh.profile.bundles.includes('@dsh-selfuse/undo');
  ...
  if (bundleMode) {
    ...
    const marker = '# @dsh-selfuse/undo mount';
    const idx = text.indexOf(marker);
    if (idx >= 0) {
      const rel = text.indexOf('name: @dsh-selfuse/undo', idx);
  ...
  if (text.includes('@dsh-selfuse/undo')) return false;
  text = text.replace(/^\s*\[\]\s*$/m, '');
  const block = `\n# @dsh-selfuse/undo mount (re-ensured by @dsh-selfuse/undo)\n- insert:\n    - id: @dsh-selfuse/undo\n      name: @dsh-selfuse/undo\n`;
...
function pickFile() {
  return new Promise((resolve) => {
    const script = [
      ...
      '$f.Title = "Select a @dsh-selfuse/undo snapshot export"',
...
function renderRestoreResult(r) {
  ...
  if (r.remounted) lines.push('@dsh-selfuse/undo mount re-ensured in cordis.patch.yml');
...
const PROMPT_TEXT = `## Undo / rollback (@dsh-selfuse/undo)
...
3. undo_restore never destroys the current state (kept as a pre-restore snapshot) and re-ensures the @dsh-selfuse/undo mount itself.
...
export function apply(ctx, config = {}) {
  ...
  void (async () => {
    try {
      ...
      if (moved > 0) ctx.logger.info(`[@dsh-selfuse/undo] migrated ${moved} legacy snapshot(s)`);
      ...
      ctx.logger.info(`[@dsh-selfuse/undo] baseline snapshot ${snap.id}${prunedN > 0 ? ` (pruned ${prunedN})` : ''}`);
    } catch (error) {
      ctx.logger.warn(`[@dsh-selfuse/undo] startup: ${String(error?.message ?? error)}`);
    }
  })();
  ...
  ctx.effect(async () => {
    try {
      ...
      ctx.logger.warn(`[@dsh-selfuse/undo] boot state failed: ${String(error?.message ?? error)}`);
      return () => { /* noop */ };
    }
  }, '@dsh-selfuse/undo.bootstate');
  ...
  })), '@dsh-selfuse/undo.tool.snapshot');
  ...
  })), '@dsh-selfuse/undo.tool.diff');

  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'undo_restore',
    description: 'Roll back DSH config to a snapshot. mode "undo" reverts the latest change (undo last action; repeats walk further back); mode "redo" re-applies the state saved before the last undo (only when nothing changed since); mode "id" restores an explicit snapshot from undo_list (restore to a fixed version). The current state is always preserved as a pre-restore snapshot first, and the @dsh-selfuse/undo mount itself is re-ensured.',
    parameters: {
  ...
  })), '@dsh-selfuse/undo.tool.export');

  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'undo_import',
    description: 'Import snapshots from a @dsh-selfuse/undo export ZIP. Snapshots are restored into the matching store by kind; same-id snapshots are skipped. The user can give you the zip path, or you can ask them to click Import in the snapshot panel.',
    parameters: {
  ...
  })), '@dsh-selfuse/undo.tool.recent');

  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'undo_safe_mode',
    description: 'Toggle SAFE MODE (v0.3): disable ALL user plugins except @dsh-selfuse/undo so DSH can always boot even when a plugin broke startup. action "on" enters (auto-snapshots first, backs up cordis.patch.yml), "off" restores the previous plugin set, "status" reports. A DSH restart is required for on/off to take effect.',
    parameters: {
    ...
    execute: async (args) => {
      ...
      if (action === 'on' || action === 'off') {
        ...
        if (!r.ok) return `undo_safe_mode failed: ${r.error}`;
        return r.message;
      }
      const st = await safeModeStatus(cfg);
      return st.active ? `Safe mode is ON (entered ${st.enteredAt ?? '?'}, backup: ${st.backup ?? '?'}).` : 'Safe mode is OFF.';
    },
  })), '@dsh-selfuse/undo.tool.safemode');

  ctx.effect(() => ctx.systemPrompt.section({
    name: 'tool:@dsh-selfuse/undo',
    order: 117,
    text: PROMPT_TEXT,
  }), '@dsh-selfuse/undo.prompt');
  ...
  const startWatcher = () => {
    ...
    const schedule = () => {
      ...
      timer = setTimeout(async () => {
        ...
        try {
          ...
          const pruned = await pruneAuto(cfg, list);
          const prunedN = pruned.removedAuto + pruned.removedPre;
          if (snap.files.length > 0 || (snap.plugins ?? []).length > 0) ctx.logger.info(`[@dsh-selfuse/undo] auto snapshot ${snap.id} (${snap.files.length} config file(s), ${(snap.plugins ?? []).length} plugin tree(s), ${reason}${prunedN > 0 ? `, pruned ${prunedN}` : ''})`);
        } catch (error) {
          ctx.logger.warn(`[@dsh-selfuse/undo] auto snapshot failed: ${String(error?.message ?? error)}`);
        }
      }, cfg.watchDebounceMs);
    };
    ...
    const watchDir = (dir) => {
      ...
      try {
        ...
        ctx.logger.warn(`[@dsh-selfuse/undo] cannot watch ${dir}: ${String(error?.message ?? error)}`);
      }
    };
      ...
      handler: async (req, res) => {
        try {
          ...
          if (method === 'POST' && path === '/api/undo/safe-mode') {
            ...
            if (action === 'on' || action === 'off') {
              return send(res, 200, { ok: true, ...await safeModeSet(cfg, action === 'on') });
            }
            const st = await safeModeStatus(cfg);
