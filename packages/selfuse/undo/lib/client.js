window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region @dsh-selfuse/undo styles
		...
		const tagId = "@dsh-selfuse/undo/undo.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@dsh-selfuse/undo";
		...
		const tagId2 = "@dsh-selfuse/undo/undo.enhance.module.css";
		...
		const KEY_STORAGE = "@dsh-selfuse/undo-keys";
		const DEFAULT_KEYS = {
		...
		const RESULT_EVENT = "@dsh-selfuse/undo-result";
		...
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "@dsh-selfuse/undo: dictionaries");
