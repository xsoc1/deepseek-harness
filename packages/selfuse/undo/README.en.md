# @dsh-selfuse/undo — Undo/rollback system for DSH
...
1. **Clone the repository** into a local plugins directory (an ASCII path is safer), e.g. `D:\dsh\plugins\@dsh-selfuse/undo`:

```bat
git clone https://github.com/lire1131/dsh-undo-plugin.git D:\dsh\plugins\@dsh-selfuse/undo
```

2. **Create a junction** so DSH's module resolver can find the local package by name `@dsh-selfuse/undo` (used by both the host plugin and the WebUI client plugin):

```bat
mklink /J "<your-dsh-install>\node_modules\@dsh-selfuse/undo" "D:\dsh\plugins\@dsh-selfuse/undo"
```
...
|---|---|
| Method A: `dsh plugin add` | `C:\Users\<your-username>\.dsh\profiles\web\node_modules\@dsh-selfuse/undo\tools\` |
| Method B: clone + junction | your clone `...\@dsh-selfuse/undo\tools\` |

> ⚠️ The install command references the repo name `dsh-undo-plugin`, but the installed folder is named after the **package name `@dsh-selfuse/undo`** — searching by "repo name" will not find it.
...
if ($d) {
  ...
  $s.TargetPath = Join-Path $d 'tools\@dsh-selfuse/undo-gui.bat'
...
```powershell
# GUI window (recommended): double-click tools\@dsh-selfuse/undo-gui.bat, or:
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo-gui.ps1"

# CLI
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo.ps1" list
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo.ps1" snapshot -Label "reason"
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo.ps1" undo
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo.ps1" redo
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo.ps1" restore -Id <id> -Force
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo.ps1" remove -Id <id>
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo.ps1" prune -KeepAuto 20
