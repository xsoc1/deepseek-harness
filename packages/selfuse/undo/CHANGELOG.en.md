...
Notable changes to @dsh-selfuse/undo. Dates are in local time (UTC+8). 中文版:[CHANGELOG.md](CHANGELOG.md)
...
### Fixed
- Documented the repo/package name mismatch: the install command says `dsh-undo-plugin`, but the installed folder is named after the **package name `@dsh-selfuse/undo`** — searching by repo name can never find it
...
### Changed
- Plugin renamed from `dsh-undo` to **`@dsh-selfuse/undo`**
