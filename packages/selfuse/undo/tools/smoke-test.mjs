// tools/smoke-test.mjs — offline smoke test of @dsh-selfuse/undo logic (no DSH needed).
...
const root = await mkdtemp(join(tmpdir(), '@dsh-selfuse/undo-test-'));
...
check((await cur('cordis.patch.yml')).includes('name: @dsh-selfuse/undo'), 'undo mount has package name');
...
const root2 = await mkdtemp(join(tmpdir(), '@dsh-selfuse/undo-test2-'));
...
const root3 = await mkdtemp(join(tmpdir(), '@dsh-selfuse/undo-test3-'));
...
const root4 = await mkdtemp(join(tmpdir(), '@dsh-selfuse/undo-test4-'));
...
await writeFile(join(profile6, 'package.json'), JSON.stringify({ name: 'dsh-profile-web', dsh: { profile: { bundles: ['@dsh-selfuse/undo'] } } }));
...
const set6 = async (v) => writeFile(join(profile6, 'package.json'), JSON.stringify({ name: 'dsh-profile-web', dsh: { profile: { bundles: ['@dsh-selfuse/undo'] } }, v }));
...
check(!patch6.includes('- id: @dsh-selfuse/undo'), 'no manual mount re-added in bundle mode');
