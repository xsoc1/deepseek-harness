...
1. **把仓库放到本地插件目录**(无中文路径更稳妥),例如 `D:\dsh\plugins\@dsh-selfuse/undo`:

```bat
git clone https://github.com/lire1131/dsh-undo-plugin.git D:\dsh\plugins\@dsh-selfuse/undo
```

2. **建立 junction**,让 DSH 的模块解析器通过包名 `@dsh-selfuse/undo` 找到本地源码(host 插件与 WebUI client 插件都靠它):
...
| 方式 B:clone + junction | 你 clone 的目录 `...\@dsh-selfuse/undo\tools\` |

> ⚠️ 注意:安装命令里的仓库名是 `dsh-undo-plugin`,但装好后目录名是**包名 `@dsh-selfuse/undo`**——按"仓库名"去找目录是找不到的。
...
```powershell
# 程序窗口(推荐):双击 tools\@dsh-selfuse/undo-gui.bat,或:
...
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\@dsh-selfuse/undo.ps1" snapshot -Label "原因"
...
- **撤销不会撤销掉自己**:恢复 `cordis.patch.yml` 后自动检查并重新写入 @dsh-selfuse/undo 挂载条。
