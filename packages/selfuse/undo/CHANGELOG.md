...
@dsh-selfuse/undo 的重要变更。日期为本地时间(UTC+8)。English version: [CHANGELOG.en.md](CHANGELOG.en.md)
...
### 修复
- 澄清包名/仓库名差异:安装命令写 `dsh-undo-plugin`,装好后目录名是**包名 `@dsh-selfuse/undo`**——按仓库名找目录必然找不到,README 已标注
...
### 变更
- 插件由 `dsh-undo` 更名为 **`@dsh-selfuse/undo`**
