...
import { subprocessRunner as sharedSubprocessRunner } from "./git-runner.js";
import { checkRefFormatArgv, classifySwitchFailure, createBranchArgv, forEachRefArgv, gitPathArgv, graphLogArgv, headBranchArgv, headShortArgv, operationMarkersArgv, OPERATION_MARKERS, statusPorcelainArgv, switchArgv, topLevelArgv, unmergedArgv, validateBranchName, verifyRefArgv, worktreeListArgv, } from "../core/git-command.js";
import { parseBranches, parseGraph, parsePorcelain, parseWorktreeBranches, } from "../core/types.js";
/**
 ...
 */
export function gitSpawnArgv(platform, argv) {
...
export class GitService {
    ...
    status(path, signal) {
        ...
        const clear = () => {
            if (this.statusFlights.get(path) === flight)
                this.statusFlights.delete(path);
        };
    ...
    async switchBranch(path, branch) {
        const gated = await this.gate(path);
        if (!gated.ok)
            return { ok: false, error: WORKSPACE_UNKNOWN };
        const root = await this.repoRoot(gated.canonical);
        if (root === null)
            return { ok: false, error: { code: 'internal', message: 'not a git repository' } };
    ...
    async operationInProgress(root, signal) {
        ...
        for (const marker of OPERATION_MARKERS) {
            ...
            if (markerPath !== '' && existsSync(resolve(root, markerPath)))
                inProgress = true;
        }
        return inProgress;
    }
