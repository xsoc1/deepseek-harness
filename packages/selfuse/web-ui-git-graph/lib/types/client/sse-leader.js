// Generated by scripts/sync-shared.mjs from shared/client/sse-leader.ts. Do not edit this copy; edit the shared source and run "node scripts/sync-shared.mjs".
/**
 ...
 */
export function subscribeSharedEvents(url, eventName, onEvent, seams = {}) {
...
function createRelay(key, url, eventName, seams) {
    ...
    if (ChannelImpl === undefined || locks === undefined) {
        ...
        source.addEventListener(eventName, (raw) => { dispatch(raw.data); });
        return { listeners, destroy: () => { source.close(); } };
    }
    const channel = new ChannelImpl('dsh-sse:' + key);
    channel.addEventListener('message', (raw) => { dispatch(raw.data); });
