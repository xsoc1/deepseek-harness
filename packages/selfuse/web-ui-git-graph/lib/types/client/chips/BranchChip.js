...
import { GraphDialog } from "../graph/GraphDialog.js";
...
function readStockLightTheme() {
    ...
    if (!body.hasAttribute(SKIN_CENTER_BODY_ATTR) || body.hasAttribute(DARK_THEME_BODY_ATTR))
        return false;
...
function useStockLightTheme() {
    ...
    useEffect(() => {
        const update = () => { setStockLightTheme(readStockLightTheme()); };
        update();
...
export function BranchChip(props) {
    ...
    useLayoutEffect(() => {
        ...
        if (anchor === null || outlet === null || stack === null || heroRow === null)
            return;
        const measure = () => {
            const stackRect = stack.getBoundingClientRect();
            const rowRect = heroRow.getBoundingClientRect();
            const anchorRect = anchor.getBoundingClientRect();
            if (stackRect.width <= 0 || rowRect.width <= 0 || anchorRect.width <= 0)
                return;
        ...
        for (const target of [anchor, outlet, stack, heroRow])
    ...
    useEffect(() => {
        ...
        const onFocus = () => {
            const now = Date.now();
            if (now - lastFocusRefetch.current < FOCUS_REFRESH_MIN_MS)
                return;
    ...
    useEffect(() => {
        if (!showBranchSelector || !branchOpen)
            return undefined;
        let live = true;
    ...
    if (!showBranchSelector || repo === undefined || repo === null)
        return null;
