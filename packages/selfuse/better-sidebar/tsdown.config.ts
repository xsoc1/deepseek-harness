/**
 * tsdown build for @dsh-selfuse/better-sidebar: the host-half lib (lib/index.js and
 ...
 *   the package-name id `@dsh-selfuse/better-sidebar` (the client-modules compose
 ...
 *   `dsh-external/@dsh-selfuse/better-sidebar` (the registry browser-side `arrive()`
 ...
 * differing: the official channel uses the package name (`@dsh-selfuse/better-sidebar`)
 * and the registry channel uses the manifest id
 * (`dsh-external/@dsh-selfuse/better-sidebar`).
 ...
 *
 * Chunk css tags use the constant plugin id `@dsh-selfuse/better-sidebar` (matching
...
function chunkBundle(name: string): UserConfig {
  return {
    ...
    noExternal: (id: string) => (CLIENT_EXTERNALS.includes(id) ? undefined : true),
    plugins: [purityGatePlugin(), makeCssPlugin('@dsh-selfuse/better-sidebar')],
    outputOptions: {
...
export default [
  ...
  clientBundle('@dsh-selfuse/better-sidebar', 'client.js'),
  // Plugin-registry channel: bundle id = manifest id (dsh.plugin.json `id`).
  clientBundle('dsh-external/@dsh-selfuse/better-sidebar', 'client-registry.js'),
