...
<div align="center">
  ...
  <img alt="@dsh-selfuse/better-sidebar workbench" src="https://github.com/user-attachments/assets/dfdb875e-a1a8-4d4b-8340-353736b1708f" />
</div>
...
# ③ Install and auto-mount (no @version = npm's latest; pin with @dsh-selfuse/better-sidebar@0.12.2)
npx -y --package @deepseek-ai/dsh dsh plugin --profile web add @dsh-selfuse/better-sidebar
```
...
# ② Allow fresh releases (once; if the key already exists, merge - @dsh-selfuse/better-sidebar under it instead)
Add-Content -Path pnpm-workspace.yaml -Value "`nminimumReleaseAgeExclude:`n  - @dsh-selfuse/better-sidebar"
...
3. Runs `dsh plugin --profile web add @dsh-selfuse/better-sidebar`: registers the dependency → detects `dsh.bundle.patch` → auto-appends the plugin to `dsh.profile.bundles`;
4. Removes any leftover hand-written mount line to avoid double-mounting (two sidebars on the page).

`curl | bash` / `irm | iex` executes remote code — the scripts are open source in the repo (`scripts/install.sh` / `scripts/install.ps1`); download and review them first if you prefer. The plugin ships as npm package `@dsh-selfuse/better-sidebar@0.12.2` and mounts via `dsh.bundle.patch` (the shipped `cordis.patch.yml`), so the DSH source is never modified.
...
```sh
dsh plugin --profile web add @dsh-selfuse/better-sidebar
```
...
2. In ~/.dsh/profiles/web/package.json dependencies write "@dsh-selfuse/better-sidebar": "link:<absolute path of the clone>"
3. Append this mount line to ~/.dsh/profiles/web/cordis.patch.yml:
   - insert:
       - id: better-sidebar
         name: '@dsh-selfuse/better-sidebar'
4. Run pnpm install in ~/.dsh/profiles/web
5. Restart DSH and hard-refresh
```

Update: `git pull && pnpm install && pnpm build` → just hard-refresh the browser (client changes hot-reload; only host-half changes need a DSH restart). To switch back to the npm channel, restore `"@dsh-selfuse/better-sidebar": "^0.12.2"` and re-run `pnpm install`.
...
dsh registry enable dsh-external/@dsh-selfuse/better-sidebar
```
...
```ts
import type {} from '@dsh-selfuse/better-sidebar'  // triggers the ctx.betterSidebar type merge
...
The dashed cards at the end of the "Sidebar content" / "File viewers" grids in the "Side Cards" settings section open the **Add tab plugins** / **Add preview plugins** modals: each declares its open extension point, offers a "**Browse more plugins on GitHub**" button (the [GitHub topic `@dsh-selfuse/better-sidebar`](https://github.com/topics/@dsh-selfuse/better-sidebar)), and lists the recommended catalog (name / repo / description / install script) — "**Open**" jumps to the repo, "**Copy**" writes the install command to the clipboard.

**Curating a new plugin**: append a `PluginEntry` to [`src/client/plugins-tabs.ts`](./src/client/plugins-tabs.ts) (tab registrations) or [`src/client/plugins-viewers.ts`](./src/client/plugins-viewers.ts) (file-previewer registrations) and tag your repo with the `@dsh-selfuse/better-sidebar` topic; data integrity is guarded by `tests/plugin-list.spec.ts`.
