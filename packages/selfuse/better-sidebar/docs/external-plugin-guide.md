# 外部插件接入指南：基于 @dsh-selfuse/better-sidebar 实现新页面
...
```ts
import type {} from '@dsh-selfuse/better-sidebar'  // 触发 declare module 'cordis' 类型合并
```
...
```jsonc
{
  ...
  "peerDependencies": {
    "cordis": "^4.0.0-rc.7",
    "@dsh-selfuse/better-sidebar": "workspace:*"
  },
  "peerDependenciesMeta": {
    "@dsh-selfuse/better-sidebar": { "optional": true }
  }
}
```

- `@dsh-selfuse/better-sidebar` 必须是 **peerDependency**（不是 dependency），避免两份实例；
...
import type {
  ...
  FileViewerProps,
  FileFetchStrategy,
} from '@dsh-selfuse/better-sidebar'

// 方式二：子路径（与主入口等价）
import type { TabDescriptor } from '@dsh-selfuse/better-sidebar/client/service'  // 别名 ./client/api
```
...
} from '@dsh-selfuse/better-sidebar/client/service'
```

> 💡 **类型合并触发路径**：`import type {} from '@dsh-selfuse/better-sidebar/client/service'` 同样会加载 `Context` 的 augmentation（`declare module 'cordis'` 在 context-types.d.ts 中）——**纯浏览器侧插件建议走 `client/service` 路径**，避免拉进宿主半的 Node 类型图（主入口 `@dsh-selfuse/better-sidebar` 的声明面含宿主代码；宿主消费者本就处于 Node 环境则无所谓）。client 可达声明图（`client/*` + context-types + html-route + prefs-shared）自 v0.12.0 起**零 Node 依赖**（`scripts/check-consumer-types.sh` 守护），没有 `@types/node`、`skipLibCheck: false` 也能编译。
...
| **双 cordis 实例** | 外部插件解析不到 DSH monorepo 的 cordis augmentation；better-sidebar 自己重述了 `interface Context { betterSidebar: ... }`（`src/context-types.ts`），你 `import type {} from '@dsh-selfuse/better-sidebar'` 即拿到类型 |
...
| **不要 value-import `@dsh-selfuse/better-sidebar`** | 即使是 `./client/service` 子路径，运行时落点也是 client bundle；只做 type-only import |
| **client 声明图零 Node 依赖（v0.12.0+）** | `@dsh-selfuse/better-sidebar/client/service` 的可达声明面（含 `Context`）不引用 `node:*` / `Buffer`——纯浏览器侧插件无需 `@types/node`，`skipLibCheck: false` 也能编译（`scripts/check-consumer-types.sh` 守护；主入口含宿主声明，宿主消费者本就处于 Node 环境） |
...
- **类型处理**：未 value-import `@dsh-selfuse/better-sidebar`，构建零耦合；与 §2 的 `import type {}` 方案可互换；
