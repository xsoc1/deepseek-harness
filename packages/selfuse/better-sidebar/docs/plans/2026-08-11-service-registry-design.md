...
将 `@dsh-selfuse/better-sidebar` 从一个"纯消费方"插件改造为"服务提供者"：
...
通过 `src/index.ts` 的 `export type * from './types.ts'` re-export 到主入口。Consumer 插件 `import type {} from '@dsh-selfuse/better-sidebar'` 触发合并。
...
| 风险 | 缓解 |
|---|---|
| 双 cordis 实例导致 consumer 拿不到 `ctx.betterSidebar` 类型 | 由 @dsh-selfuse/better-sidebar 自己 `declare module 'cordis'`，consumer `import type {}` 触发合并 |
