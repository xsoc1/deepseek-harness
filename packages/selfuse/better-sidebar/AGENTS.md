# @dsh-selfuse/better-sidebar 插件接入文档
...
- **类型合并**：`declare module 'cordis' { interface Context { betterSidebar: BetterSidebarService } }` 由 `@dsh-selfuse/better-sidebar` 包导出；消费插件 `import type {} from '@dsh-selfuse/better-sidebar'` 即触发类型合并
...
- `@dsh-selfuse/better-sidebar` 必须声明为 **peerDependency**（不是 dependency，避免重复实例化）
...
```ts
import type { TabDescriptor, FileViewerDescriptor, BetterSidebarService } from '@dsh-selfuse/better-sidebar'
```
