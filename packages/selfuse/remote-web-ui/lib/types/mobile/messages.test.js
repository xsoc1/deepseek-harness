...
function textChunk(turn, step, text, seq) {
...
describe('foldEvents', () => {
    ...
    it('keeps the streamed pending assistant alive while chunks arrive, then finalizes', () => {
        ...
        const finalized = foldEvents([makeEvent('assistant/message', assistantMessageData('a-1', 0, 0, '你好！'), 3)], first);
    ...
    it('keeps streamed reasoning when the final assistant message omits the reasoning block', () => {
        ...
        const finalized = foldEvents([makeEvent('assistant/message', assistantMessageData('a-1', 0, 0, '回答'), 2)], streamed);
        const assistant = finalized.find(message => message.kind === 'assistant');
