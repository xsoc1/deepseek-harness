 ...
 */
export function shouldRewriteFetchPath(pathname) {
    if (pathname.startsWith(PAIR_PREFIX))
        return false;
    if (pathname.startsWith(UPDATE_PREFIX))
        return false;
    if (pathname === DESKTOP_LAUNCHER_PREFIX || pathname.startsWith(`${DESKTOP_LAUNCHER_PREFIX}/`))
        return false;
    if (pathname === SETTINGS_BRIDGE_PREFIX || pathname.startsWith(`${SETTINGS_BRIDGE_PREFIX}/`))
        return false;
    if (pathname.startsWith(API_PREFIX))
        return true;
    if (pathname.startsWith(SIDEBAR_PREFIX) || pathname === '/sidebar')
        return true;
    if (pathname.startsWith(GIT_PREFIX) || pathname === '/git')
        return true;
    if (pathname.startsWith(PET_PREFIX) || pathname === '/pet')
        return true;
    return false;
}
/**
 ...
 */
export function shouldRewriteWsPath(pathname) {
 ...
 */
export function rewriteRawUrl(raw, baseHref, origin) {
    let url;
    try {
    ...
    if (url.origin !== origin)
        return raw;
    if (!shouldRewriteFetchPath(url.pathname))
        return raw;
 ...
 */
export async function isUnpairedDenied(response) {
 ...
 */
export function installRemoteChannel(window, options = {}) {
    ...
    const patchedFetch = (input, init) => {
        ...
        if (sameOrigin(url) && shouldRewriteFetchPath(url.pathname)) {
            ...
            return Promise.resolve(originalFetch.call(window, next, init)).then(async (response) => {
                if (await isUnpairedDenied(response.clone()))
                    options.onUnpaired?.();
                else
                    options.onPaired?.();
                return response;
    ...
    return () => {
        ...
        for (const restore of restoreSrc)
            restore();
    };
}
/**
 ...
 */
export function channelTransition(active, installed) {
    if (active && !installed)
        return 'install';
    if (!active && installed)
        return 'retire';
