...
import { copyText, issuePair, revokePair, stopPair } from "./pair-api.js";
...
export function RemoteEntry({ wide, useWorkspaces, t }) {
    ...
    const openPanel = useCallback(async () => {
        ...
        if (next.kind !== 'ready' && next.kind !== 'lan-required')
            return;
...
function TooltipAnchor({ wide, label, onClick }) {
    return (_jsx("button", { type: "button", className: css.trigger, "data-wide": wide ? 'wide' : 'rail', "aria-label": label, title: label, onClick: onClick, children: _jsx(PhoneIcon, { size: wide ? 16 : 18 }) }));
}
