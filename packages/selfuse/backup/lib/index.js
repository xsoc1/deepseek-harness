...
export const name = '@dsh-selfuse/backup';
...
export function apply(ctx, pluginConfig) {
  ...
  async function githubSync(signal) {
    ...
    await spawnRun([git, '-c', 'user.name=@dsh-selfuse/backup', '-c', 'user.email=@dsh-selfuse/backup@users.noreply.github.com', 'commit', '-m', message], syncDir, signal);
  ...
  async function runAutoBackup() {
    try {
      ...
      console.error(`[@dsh-selfuse/backup] 自动备份失败: ${String(err && err.message ? err.message : err)}`);
    }
  }
