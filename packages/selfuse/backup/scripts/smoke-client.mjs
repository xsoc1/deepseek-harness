...
async function main() {
  ...
  ok(entry !== undefined && entry.id === '@dsh-selfuse/backup', `bundle 以 id=${entry?.id} 注册`);
  ...
  ok(plugin.name === '@dsh-selfuse/backup' && JSON.stringify(plugin.inject) === JSON.stringify(['slots', 'locale', 'remote']), `插件导出面正确: ${plugin.name}`);
  ...
  ok(contribution?.package === '@dsh-selfuse/backup' && contribution.descriptors.length === 9, `Remote 贡献含 ${contribution?.descriptors.length} 个端点`);
  ...
  const samples = {
    'backupPanel/status': {
      destination: 'C:/Users/u/Desktop/@dsh-selfuse/backups', dshHome: 'C:/Users/u/.dsh',
