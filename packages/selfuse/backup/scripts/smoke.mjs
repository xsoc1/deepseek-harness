/**
 * @dsh-selfuse/backup 冒烟测试（零依赖，跨平台）。
 *
...
async function mkTmpHome() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), '@dsh-selfuse/backup-smoke-'));
  const home = path.join(dir, 'home');
  const root = `${home}${path.sep}Desktop${path.sep}@dsh-selfuse/backups`;
...
async function main() {
  ...
  try {
    const config = { destination: `~/Desktop/@dsh-selfuse/backups`, keep: 7 };
    ...
    ok(contrib !== undefined && contrib.package === '@dsh-selfuse/backup' && contrib.face === 'host', 'typert 贡献已注册（host 面）');
    ...
    if (panel) {
      ...
      ok(snap.destination.includes('Desktop/@dsh-selfuse/backups') && snap.dshHome.endsWith('/.dsh') && Number.isInteger(snap.keepDefault), `status 快照: dest=${snap.destination}`);
    ...
    const syncDir2 = path.join(home, 'Desktop', '@dsh-selfuse/backups', '.github-sync');
