window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		const STYLE_TAG_DATA_ATTRIBUTE = "data-plugin=\"@dsh-selfuse/wsl-workspace\"";
		const STYLES = `
...
`;
		...
		*/
		function ensureStyles() {
			...
			style.setAttribute("data-plugin", "@dsh-selfuse/wsl-workspace");
		...
		*/
		function apply(ctx) {
			...
			ctx.effect(() => ctx.locale.register("wslWorkspace", {
				zh,
				en
			}), "@dsh-selfuse/wsl-workspace: locale dictionaries");
			...
			ctx.effect(() => ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
				...
				id: "wsl-workspace",
				inject: injected
			}, AddWslWorkspace)), "@dsh-selfuse/wsl-workspace: sidebar footer action");
			ctx.effect(() => {
			...
			}, "@dsh-selfuse/wsl-workspace: WSL mode-variant binding");
		}
		//#endregion
		exports.apply = apply;
