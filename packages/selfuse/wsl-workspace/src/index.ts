/**
 * Host half of @dsh-selfuse/wsl-workspace. Three responsibilities:
 *
 ...
 * @module @dsh-selfuse/wsl-workspace
 */
...
export const name = '@dsh-selfuse/wsl-workspace'
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (agentPresets !== undefined) {
    ctx.effect(() => {
      void materializeVariants(agentPresets, dshHome, shellPath, fsPath).catch((error) => {
        ...
        console.error(`@dsh-selfuse/wsl-workspace: WSL preset-variant generation failed: ${messageOf(error)}`)
      })
      return () => {}
    }, '@dsh-selfuse/wsl-workspace: WSL preset variants')
  }
  ...
  if (shellEnv !== undefined) {
    ...
    }), '@dsh-selfuse/wsl-workspace: per-session distro env fact')
  }
  ...
  }), '@dsh-selfuse/wsl-workspace: dialog data route')
}
