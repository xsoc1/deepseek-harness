 ...
 * @param pluginArgs - the raw args, e.g. ['add', '@dsh-selfuse/market@latest'].
...
export function classifyPnpmFailure(output: string): PnpmFailure | null {
  ...
  if (output.includes('ERR_PNPM_ADDING_TO_ROOT')) {
    return {
      code: 'adding-to-root',
      recoverable: false,
      message: 'pnpm 拒绝在 workspace 根目录安装（缺少 -w）。这是市场的 bug，请升级 @dsh-selfuse/market 到最新版 / pnpm refused to add at a workspace root (missing -w); this is a market bug — please update @dsh-selfuse/market',
    }
  }
  if (/--workspace-root may only be used inside a workspace/i.test(output)) {
    return {
      ...
      recoverable: false,
      message: 'profile 目录不是 pnpm workspace，却传入了 -w。这是市场的 bug，请升级 @dsh-selfuse/market 到最新版 / -w was passed but the profile is not a pnpm workspace; this is a market bug — please update @dsh-selfuse/market',
    }
  }
