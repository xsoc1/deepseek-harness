...
const SELF_NAMES = new Set(['@dsh-selfuse/market', 'dsh-market']);
/**
...
export function mountMarketRoutes(host, config, commandRuntime, agentsLookup) {
    ...
    const disposers = [
            ...
            handler: (request, response) => {
                ...
                try {
                    ...
                    response.writeHead(200, {
                        ...
                        'content-disposition': `attachment; filename="dsh-@dsh-selfuse/market-backup-${timestamp}.json"`,
                    });
                    response.end(backup);
                }
                catch (error) {
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    if (name === 'dsh-market' || name === '@dsh-selfuse/market') {
         ...
         * never as a stray `{ name: "@dsh-selfuse/market" }` on the ordinary path.
         *
            ...
            handler: async (request, response) => {
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        const installed = readInstalled(config.profile, activeProfileDir);
                        const selfName = ['@dsh-selfuse/market', 'dsh-market'].find(candidate => installed[candidate] !== undefined);
