 ...
 */
export function hasApiRemoteSubagentOwner(ctx: any, session: any, agent: any) {
 ...
 */
export function apiRemoteSubagentOwnershipError(sessionId: string) {
  return {
    code: 'agent-busy',
 ...
 */
export async function inspectApiRemoteSession(ctx: any, sessionId: string) {
  const persistence = ctx.get('sessionPersistence')
  if (persistence === undefined) {
    throw new Error('session persistence is not configured (load a dsh-session-persistence backend)')
  }
  const meta = (await persistence.list()).find((candidate: any) => candidate.id === sessionId)
 ...
 */
export function createApiRemoteAgentResolver(ctx: any, options: any) {
  const resumes = new Map()
  const fencedLiveAgent = (sessionId: string) => {
  ...
  ctx.inject(['typert'], (typeCtx: any) => {
    ...
    typeCtx.typert.lookups.configure('session', async (sessionId: string) => (await resolveAgent(sessionId)).session)
    typeCtx.typert.contexts.configureHost('agent', async (sessionId: string) => (await resolveAgent(sessionId)).ctx)
  })
  return agentFor
}
