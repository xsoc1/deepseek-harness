...
$missingBundles = @($clientBundles | Where-Object { -not (Test-Path $_) })
if ($missingBundles.Count -gt 0) {
    Add-Content -LiteralPath "F:\tools\deepseek-harness\dsh-web.log" -Value "preflight: missing client bundles ($($missingBundles -join ', ')); rebuilding" -Encoding UTF8
    $pnpm = Join-Path (Get-Location).Path 'node_modules\.bin\pnpm.cmd'
